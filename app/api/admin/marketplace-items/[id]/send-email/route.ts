...
import { resolveMarketplaceSellerEmail } from '@/lib/email/resolveMarketplaceSellerEmail';
...
/**
 * POST: send a one-off message to the seller’s account email for an individual marketplace listing.
...
export async function POST(req: Request, context: RouteContext) {
  try {
    ...
    if (row.archived_at) {
      return NextResponse.json({ error: 'This listing is archived and cannot receive marketplace emails here.' }, { status: 400 });
    }

    const userId = typeof row.user_id === 'string' ? row.user_id : null;
    const toEmail = await resolveMarketplaceSellerEmail(supabaseAdmin, userId);

    if (!toEmail) {
      return NextResponse.json(
        { error: 'No seller email on file (account may be missing or has no login email).' },
        { status: 400 }
      );
    }

    const listingTitle = String(row.title || 'Your listing').trim() || 'Your listing';
...
<html><body style="font-family: system-ui, sans-serif; line-height: 1.55; color: #111; max-width: 640px;">
  <p>Hi,</p>
  <p style="color:#4b5563;font-size:14px;">This message is about your Hanar marketplace listing: <strong>${safeTitle}</strong></p>
  ...
  <p style="color:#9ca3af;font-size:12px;margin-top:24px;">— Hanar Support</p>
    ...
    const result = await sendHanarEmail({
      ...
      subject,
      html,
      text: `Hi,\n\nRegarding your Hanar marketplace listing "${listingTitle}":\n\n${message}\n\n— Hanar Support\nsupport@hanar.net`,
      tags: [
        { name: 'template', value: 'admin_marketplace_item_direct' },
        { name: 'marketplace_item_id', value: itemId },
      ],
      logUserId: ownerId,
    });
    ...
    console.error('admin marketplace item send-email error:', err instanceof Error ? err.name : 'unknown');
    return NextResponse.json(
