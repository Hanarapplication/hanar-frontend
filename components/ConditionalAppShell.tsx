...
export default function ConditionalAppShell({
  ...
  if (isBusinessSlugPage || isAuthPage || isAdminRoute) {
     ...
     * Business slug pages: no top padding so the gallery/hero is flush to the top. */
    const shellClass = isBusinessSlugPage
      ? 'animate-route-swap-soft pt-0'
        ...
        className={`animate-route-swap-soft ${
          isHomeFeedShell
            ? `pt-[calc(4rem+env(safe-area-inset-top,0px))] ${
                homeBottomBarDocked
                  ? 'pb-[calc(7.75rem+env(safe-area-inset-bottom,0px)+1rem)]'
