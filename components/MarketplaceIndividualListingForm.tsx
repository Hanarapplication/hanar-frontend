...
export function MarketplaceIndividualListingForm({ editItemId }: { editItemId?: string }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (error || !user) {
        const redirect = isEditMode && editItemId ? `/marketplace/edit/${editItemId}` : '/marketplace/post';
      ...
      if (isEditMode && editItemId) {
        ...
        try {
          ...
          const res = await fetch(`/api/marketplace/my-listing/${encodeURIComponent(editItemId)}`, { headers });
          const data = await res.json().catch(() => ({}));
          if (!res.ok) {
            toast.error(data?.error || 'Could not load listing');
            router.push('/dashboard');
            return;
          }
          const it = data.item as FormState & { id?: string };
          setForm({
            title: it.title ?? '',
            price: it.price ?? '',
            ...
            locationLat: it.locationLat ?? null,
            locationLng: it.locationLng ?? null,
            ...
            externalBuyUrl: it.externalBuyUrl ?? '',
          });
          setImagePreviews(Array.isArray(it.imageUrls) ? it.imageUrls : []);
        } finally {
    ...
    load();
  }, [router, isEditMode, editItemId]);

  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
    if (name.startsWith('contact.')) {
      const key = name.split('.')[1] as keyof FormState['contact'];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isEditMode && limitReached && !isBusiness) {
      return toast.error('Listing limit reached. Delete one from your dashboard or get the Casual Seller Pack to list more.');
    }
    ...
    if (trimmedUrl) payload.external_buy_url = trimmedUrl;
    else payload.external_buy_url = null;
    if (locationCity) payload.location_city = locationCity;
    if (locationState) payload.location_state = locationState;
    if (locationCountry) payload.location_country = locationCountry;
    if (locationZip) payload.location_zip = locationZip;
    ...
    if (isEditMode && editItemId) {
      const res = await fetch(`/api/marketplace/my-listing/${encodeURIComponent(editItemId)}`, {
        method: 'PATCH',
        headers,
      ...
      if (!res.ok) {
        ...
        router.push(`/marketplace/individual-${editItemId}`);
      }
      return;
    }
  ...
  const isDisabled = uploading || submitting || (!isEditMode && limitReached && !isBusiness);

  if (editLoading) {
    return (
      <div className="min-h-screen bg-slate-50 py-16 text-center text-slate-600">Loading listing…</div>
    );
  }
      ...
      <div className="max-w-xl mx-auto px-4 sm:px-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 mb-1">
            {isEditMode ? 'Edit your listing' : 'Post an Item for Sale'}
          </h1>

          <p className="text-sm text-slate-500 mb-6">
            {isEditMode
              ? 'Update details and save. Your listing stays visible while you edit.'
              : isBusiness
                ? 'Unlimited posts'
                : 'Choose a package below. Free: 1 listing (30 days). Or get the Casual Seller Pack for up to 5 listings.'}
          </p>

          {!isEditMode && limitReached && !isBusiness && (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <MarketplaceCategorySelects
              ...
              labelId={isEditMode ? 'edit-item-category' : 'post-item-category'}
              className="space-y-2"
            />
            <div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3 mt-2">
                {imagePreviews.map((src, i) => (
                  <div key={`${src}-${i}`} className="relative w-20 h-20 rounded-xl overflow-hidden border border-slate-200">
                    <img src={src} className="object-cover w-full h-full" alt="" />
                    <button type="button" onClick={() => removeImage(i)} className="absolute top-1 right-1 w-5 h-5 flex items-center justify-center bg-red-500 text-white text-xs rounded-full hover:bg-red-600">
                      ×
                    </button>
                  </div>
                ))}
            ...
            >
              {uploading ? 'Uploading...' : submitting ? (isEditMode ? 'Saving...' : 'Posting...') : isEditMode ? 'Save changes' : 'Post Item'}
            </button>
          </form>
        </div>
      </div>
